...
if !exists(global.nozzleDiameterInstalled)  ; checks for the existence of global.nozzleDiameterInstalled
	global nozzleDiameterInstalled = 0.8       ; if it doesn't exist, set the value to 0.4mm, which is the default for the Troodon V2
...
if !exists(global.daemon)                   ; checks for the existence of global.generatePrintOnlyMesh
	global daemon = false                      ; if it doesn't exist, set the value to false
if !exists(global.heightchanges)                   ; checks for the existence of global.generatePrintOnlyMesh
	global heightchanges = false                      ; if it doesn't exist, set the value to false